...
st.par <- c("Florida","Illinois","California","Colorado",
            "New Mexico","Oregon","Massachusetts","Indiana","Michigan",
            "Utah","South Dakota","Idaho","Nebraska","Maine",
            "North Carolina","New Hampshire","Texas","New York")

par.states <- States[States$NAME_1 %in% st.par,]
st.center <- rgeos::gCentroid(par.states,byid = TRUE)@coords
AK <- cbind(-149.4937,64.2008)
anchorage <- cbind(-150.000689,61.1077052)
#BC <- cbind(-127.6476,53.7267)
MTL <- cbind(-74.0104841,45.5576996)
ON <- cbind(-85.3232,51.2538)
PR <- cbind(-66.5901,18.2208)
MX <- cbind(-99.2836991,19.39068)
...
coord <- rbind(st.center,ON,PR,AK,MX,MTL)
...
gcLines <- geosphere::gcIntermediate(par.locs,anchorage,n=50,addStartEnd=TRUE)
...
raster::plot(sp::spTransform(sp::SpatialPoints(anchorage,sp::CRS("+init=epsg:4326")),
